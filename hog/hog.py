...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, average_score = 0, 0
    for num in range(1, 11):
        outcome = make_averaged(roll_dice)(num, dice)
        if outcome > average_score:
            average_score = outcome
            num_rolls = num
    return num_rolls
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    y = score + max(opponent_score // 10, opponent_score % 10) + 1
    z = 1 + max(opponent_score // 10, opponent_score % 10)
    if opponent_score % y == 0:
        return 0
    elif z >= margin:
        return 0
    else:
