...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(i) for i in clusters if len(i) != 0]
        # END Question 6
        n += 1
    return centroids
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    rating = lambda restaurant: user_rating(user, restaurant_name(restaurant)) if restaurant in reviewed else predictor(restaurant)
