...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('unequal bindings to symbols and functions')
        while formals != nil:
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        args = operands.map(lambda procedure: scheme_eval(procedure, env))
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Invalid funciton call')
        # END PROBLEM 4
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        "*** YOUR CODE HERE ***"
        expression = complete_eval(scheme_apply(self, operands, env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        user_defined = do_lambda_form(Pair(expressions.first.second, expressions.second), env)
        env.bindings[target.first] = user_defined
        return target.first
        # END PROBLEM 10
    else:
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        user_defined = MacroProcedure(expressions.first.second, expressions.second, env)
        env.bindings[expressions.first.first] = user_defined
